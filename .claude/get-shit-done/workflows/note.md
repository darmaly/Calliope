...
- **Global scope**: `/Users/deebarmaly/AI_DAW/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/deebarmaly/AI_DAW/.claude/notes/`)
...
2. Glob `/Users/deebarmaly/AI_DAW/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/deebarmaly/AI_DAW/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/deebarmaly/AI_DAW/.claude/notes/` — works in any directory
